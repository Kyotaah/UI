...
local isStudio
local website = "github.com/Kyotaah"

if RunService:IsStudio() then
	isStudio = true
end

-- Credits To Latte  And qweery for Lucide And Material Icons Respectively.
local IconModule = {
	Lucide = nil,
	Material = {
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		GUIHolder:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	StarvineUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(StarvineUI)
	StarvineUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	StarvineUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == StarvineUI.Name and Interface ~= StarvineUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "Starvine-Old"
		end
	end
elseif not isStudio then
...
StarvineUI.Enabled = false
StarvineUI.SmartWindow.Visible = false
StarvineUI.Notifications.Template.Visible = false
StarvineUI.DisplayOrder = 1000000000

local Main : Frame = StarvineUI.SmartWindow
local Dragger = Main.Drag
local dragBar = StarvineUI.Drag
...
local Notifications = StarvineUI.Notifications
...
-- 	for FlagName, Flag in pairs(GUIHolder.Flags) do
...
-- 		else
-- 			notified = true
-- 			Starvine:Notification({Title = "Config Error", Content = "Starvine was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		Starvine:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using Starvine Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		Starvine:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using Starvine Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(GUIHolder.Flags) do
...
function Starvine:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Starvine:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Starvine UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "Starvine Interface Suite",
		LoadingSubtitle = "by Kyotaah ",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Starvine Interface Suite" and Release or "Starvine UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(StarvineUI.MobileSupport, StarvineUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Starvine/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Starvine/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Starvine") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Starvine"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Starvine", FoundKey)
						end
						Starvine:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			...
			DiscordInvite = "noinvitelink" -- The disvord invite link. Do not include the link so for example if my invite was discord.gg/Kyotaah I would put Kyotaah
		}, HomeTabSettings or {})
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Starvine Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Starvine Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Starvine Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- GUIHolder.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					GUIHolder.Options[Flag] = SliderV
				end

				StarvineUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = GUIHolder.ThemeGradient
					Slider.Main.UIStroke.color.Color = GUIHolder.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Starvine Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				StarvineUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = GUIHolder.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = GUIHolder.ThemeGradient
				end)

				if Flag then
					GUIHolder.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Starvine Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					GUIHolder.Options[Flag] = BindV
				end

				-- GUIHolder.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Starvine Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Starvine Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- GUIHolder.Flags[DropdownSettings.Flag] = DropdownSettings
				...
				if Flag then
					GUIHolder.Options[Flag] = DropdownV
				end
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Starvine Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					GUIHolder.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Starvine:SaveConfig(inputPath)
					if not success then
						Starvine:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Starvine:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = Starvine:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Starvine:LoadConfig(selectedConfig)
					if not success then
						Starvine:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Starvine:SaveConfig(selectedConfig)
					if not success then
						Starvine:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(GUIHolder.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(GUIHolder.Folder .. "/settings/autoload.txt")
			...
			if isfile(GUIHolder.Folder .. "/settings/autoload.txt") then
				local name = readfile(GUIHolder.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if GUIHolder.Options[Flag] then
						GUIHolder.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if GUIHolder.Options[Flag] and data.value then
						GUIHolder.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if GUIHolder.Options[Flag] and data.text and type(data.text) == "string" then
						GUIHolder.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if GUIHolder.Options[Flag] and data.value then
						GUIHolder.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if GUIHolder.Options[Flag] and data.color then
						GUIHolder.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "StarvineInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "StarvineInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "StarvineInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						GUIHolder.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						StarvineUI.ThemeRemote.Value = not StarvineUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						GUIHolder.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						StarvineUI.ThemeRemote.Value = not StarvineUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						GUIHolder.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						StarvineUI.ThemeRemote.Value = not StarvineUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				GUIHolder.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				GUIHolder.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Starvine:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = GUIHolder.Folder .. "/settings/" .. Path .. ".Starvine"

			local data = {
				objects = {}
			}

			for flag, option in next, GUIHolder.Options do
		...
		function Starvine:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = GUIHolder.Folder .. "/settings/" .. Path .. ".Starvine"
		...
		function Starvine:LoadAutoloadConfig()
			if isfile(GUIHolder.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Starvine:LoadConfig(name)
				if not success then
					return Starvine:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Starvine:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(GUIHolder.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Starvine" then
					local pos = file:find(".Starvine", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			StarvineUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			StarvineUI.MobileSupport.Visible = false
	...
	StarvineUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		StarvineUI.MobileSupport.Visible = false
...
if (getgenv and not getgenv().ConfirmStarvine) or (not getgenv) then
	Starvine:Notification({ 
    	Title = "Starvine Is Deprecated",
    	Icon = "warning",
    	ImageSource = "Material",
    	Content = "If you are not the script developer, ignore this message. \n\n The Starvine Interface Library Is Deprecated And Not Recommended to Use. A New Library Is Available at Kyotaah.xyz/starlight. If you insist on using Starvine, set the getgenv().ConfirmStarvine variable to true. "
	})
end

if isStudio then
	local Window = Starvine:CreateWindow({
		Name = "Kyotaah Client - Starvine Hub | Blade Ball",
		Subtitle = "by Kyotaah ",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Kyotaah Client (Starvine Hub)",
		LoadingSubtitle = "Loading script for Blade Ball",
		KeySystem = true,
		KeySettings = {
			Title = "Kyotaah Client | Key System",
			Subtitle = "Blade Ball",
			Note = "Please Enter Your Key To Use Kyotaah Client",
	...
	Starvine:Notification({ 
		Title = "Welcome to Starvine",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the Starvine Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Kyotaah . Starvine was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Starvine Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Starvine Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "Starvine's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = Starvine:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the Starvine Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Kyotaah . Starvine was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Starvine Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Starvine Today at " .. website})
