local Release = "Prerelease Beta v4.0.1"
...
local isStudio
local website = "github.com/Hanto-Softworks"
...
local HantoUI = isStudio and script.Parent:WaitForChild("Hanto UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Hanto:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	HantoUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(HantoUI)
	HantoUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	HantoUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == HantoUI.Name and Interface ~= HantoUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "Hanto-Old"
		end
	end
elseif not isStudio then
...
HantoUI.Enabled = false
HantoUI.SmartWindow.Visible = false
HantoUI.Notifications.Template.Visible = false
HantoUI.DisplayOrder = 1000000000

local Main : Frame = HantoUI.SmartWindow
local Dragger = Main.Drag
local dragBar = HantoUI.Drag
...
local Notifications = HantoUI.Notifications
...
-- 	for FlagName, Flag in pairs(Hanto.Flags) do
...
-- 		else
-- 			notified = true
-- 			Hanto:Notification({Title = "Config Error", Content = "Hanto was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		Hanto:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using Hanto Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		Hanto:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using Hanto Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(Hanto.Flags) do
...
function Hanto:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Hanto:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Hanto UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "Hanto Interface Suite",
		LoadingSubtitle = "by Hanto Softworks",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Hanto Interface Suite" and Release or "Hanto UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(HantoUI.MobileSupport, HantoUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Hanto/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Hanto/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Hanto") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Hanto"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Hanto", FoundKey)
						end
						Hanto:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			...
			DiscordInvite = "noinvitelink" -- The disvord invite link. Do not include the link so for example if my invite was discord.gg/Hanto I would put Hanto
		}, HomeTabSettings or {})
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Hanto Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Hanto Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Hanto Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- Hanto.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					Hanto.Options[Flag] = SliderV
				end

				HantoUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = Hanto.ThemeGradient
					Slider.Main.UIStroke.color.Color = Hanto.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Hanto Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				HantoUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = Hanto.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = Hanto.ThemeGradient
				end)

				if Flag then
					Hanto.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Hanto Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					Hanto.Options[Flag] = BindV
				end

				-- Hanto.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Hanto Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Hanto Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- Hanto.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Hanto Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					Hanto.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Hanto:SaveConfig(inputPath)
					if not success then
						Hanto:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Hanto:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = Hanto:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Hanto:LoadConfig(selectedConfig)
					if not success then
						Hanto:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Hanto:SaveConfig(selectedConfig)
					if not success then
						Hanto:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(Hanto.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(Hanto.Folder .. "/settings/autoload.txt")
			...
			if isfile(Hanto.Folder .. "/settings/autoload.txt") then
				local name = readfile(Hanto.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if Hanto.Options[Flag] then
						Hanto.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if Hanto.Options[Flag] and data.value then
						Hanto.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if Hanto.Options[Flag] and data.text and type(data.text) == "string" then
						Hanto.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if Hanto.Options[Flag] and data.value then
						Hanto.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Hanto.Options[Flag] and data.color then
						Hanto.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "HantoInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "HantoInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "HantoInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						Hanto.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						HantoUI.ThemeRemote.Value = not HantoUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						Hanto.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						HantoUI.ThemeRemote.Value = not HantoUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						Hanto.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						HantoUI.ThemeRemote.Value = not HantoUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Hanto.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				Hanto.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Hanto:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = Hanto.Folder .. "/settings/" .. Path .. ".Hanto"

			local data = {
				objects = {}
			}

			for flag, option in next, Hanto.Options do
		...
		function Hanto:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = Hanto.Folder .. "/settings/" .. Path .. ".Hanto"
		...
		function Hanto:LoadAutoloadConfig()
			if isfile(Hanto.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Hanto:LoadConfig(name)
				if not success then
					return Hanto:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Hanto:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(Hanto.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Hanto" then
					local pos = file:find(".Hanto", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			HantoUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			HantoUI.MobileSupport.Visible = false
	...
	HantoUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		HantoUI.MobileSupport.Visible = false
...
if (getgenv and not getgenv().ConfirmHanto) or (not getgenv) then
	Hanto:Notification({ 
    	Title = "Hanto Is Deprecated",
    	Icon = "warning",
    	ImageSource = "Material",
    	Content = "If you are not the script developer, ignore this message. \n\n The Hanto Interface Library Is Deprecated And Not Recommended to Use. A New Library Is Available at Hantosoftworks.xyz/starlight. If you insist on using Hanto, set the getgenv().ConfirmHanto variable to true. "
	})
end

if isStudio then
	local Window = Hanto:CreateWindow({
		Name = "Hanto Client - Hanto Hub | Hanto!",
		Subtitle = "by Hanto Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Hanto Client (Hanto Hub)",
		LoadingSubtitle = "Loading script for Hanto!",
		KeySystem = true,
		KeySettings = {
			Title = "Hanto Client | Key System",
			Subtitle = "Hanto!",
			Note = "Please Enter Your Key To Use Hanto Client",
	...
	Hanto:Notification({ 
		Title = "Welcome to Hanto",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the Hanto Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Hanto Softworks. Hanto was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Hanto Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Hanto Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "Hanto's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = Hanto:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the Hanto Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Hanto Softworks. Hanto was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Hanto Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Hanto Today at " .. website})
